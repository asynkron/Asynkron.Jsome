...
class Program
{
    static async Task<int> Main(string[] args)
    {
        ...
        // Create root command
        var rootCommand = new RootCommand("Asynkron.Jsome - Advanced Swagger 2.0 and JSON Schema Code Generator")
        {
    ...
    {
        var rule = new Rule("[bold blue]Asynkron.Jsome - Advanced Swagger 2.0 and JSON Schema Code Generator[/]")
        {
...
""");
        ...
        AnsiConsole.WriteLine();
        AnsiConsole.MarkupLine("[dim]For more examples, see: src/Asynkron.Jsome/Samples/Configuration/sample-config.yaml[/]");
    }
    ...
    private static string? FindSourceDirectory(string currentDirectory)
    {
        ...
        {
            var srcDir = Path.Combine(directory.FullName, "src", "Asynkron.Jsome");
